package com.mecui.app.web.rest;
...
import com.mecui.app.IntegrationTest;
import com.mecui.app.config.EmbeddedKafka;
import com.mecui.app.config.KafkaSseConsumer;
import com.mecui.app.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class MasterDataCatalogueUiKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/master-data-catalogue-ui-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/master-data-catalogue-ui-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/master-data-catalogue-ui-kafka/unregister"));
                return;
            }
        }
